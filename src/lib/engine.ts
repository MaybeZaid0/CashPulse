...
export function calculateCashPulseScore(
  ...
  const inflows = sme.historyInflows;
  const outflows = sme.historyOutflows;
  ...
  const reserveRatio = sme.initialBalance / (avgOutflow || 1);
  ...
  const regularitySubscore = sme.paymentRegularity;
  ...
  let runningBalance = sme.initialBalance;
  ...
  // Forecast Months 7-9
  for (let k = 1; k <= 3; k++) {
    const multIn = sme.seasonalMultIn[k - 1];
    const multOut = sme.seasonalMultOut[k - 1];
