  ...
  return (
    <div className="space-y-6 pb-12">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between bg-white p-4 rounded-2xl border border-[#E4EBF2] shadow-sm gap-3">
        <button
          onClick={onBack}
          className="flex items-center space-x-2 text-xs font-bold text-[#0E1B2A] hover:text-[#0083CA] px-3 py-1.5 rounded-xl bg-[#F4F7FB] border border-[#E4EBF2] transition-all"
        >
        ...
        <div className="flex items-center space-x-3">
          <button
            ...
            className="flex items-center space-x-1.5 px-3 py-1.5 rounded-xl text-xs font-bold bg-[#F4F7FB] border border-[#E4EBF2] text-[#0083CA] hover:bg-[#0083CA]/10 transition-all"
          >
      ...
      <div className="bg-[#012A4A] text-white rounded-2xl p-6 shadow-xl border border-slate-800">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="flex items-center space-x-3">
            <div className="w-12 h-12 rounded-xl bg-[#0083CA]/20 border border-[#0083CA]/30 flex items-center justify-center text-[#00B7E4]">
              <Building2 className="w-6 h-6" />
            </div>
            <div>
              <div className="flex items-center space-x-2">
                <h1 className="text-xl font-bold text-white">{currentApp.smeName}</h1>
                <span className="text-[11px] font-mono text-[#F2A900] font-bold bg-[#F2A900]/10 px-2 py-0.5 rounded border border-[#F2A900]/20">
                  {currentApp.id}
                </span>
              </div>
          ...
          <div className="grid grid-cols-2 gap-3 text-xs">
            ...
            <div className="bg-slate-900/80 p-3 rounded-xl border border-slate-800">
              ...
              <span className="font-extrabold text-[#00B7E4] text-sm">
      ...
      {/* Score Ring & Recommendation Banner */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        {/* Automated Recommendation Panel (F-8, GR-1, GR-2) */}
        <div className="md:col-span-2 bg-white rounded-2xl p-6 border border-[#E4EBF2] shadow-sm space-y-4 flex flex-col justify-between">
          <div>
            <div className="flex items-center justify-between border-b border-[#E4EBF2] pb-3">
              <div className="flex items-center space-x-2">
                ...
                <h3 className="font-bold text-[#0E1B2A] text-base">
                  Automated Recommendation (GR-1 Human Advisory)
                </h3>
              </div>
            ...
            <div className="pt-3 space-y-3">
              ...
              <div className="bg-[#F4F7FB] p-3.5 rounded-xl border border-[#E4EBF2] space-y-1.5">
                <span className="text-[11px] font-bold text-[#0E1B2A] block uppercase">
                  Evidence Audit Trail (GR-2):
                </span>
                {recommendation.evidence.map((item, idx) => (
                  <div key={idx} className="text-xs text-[#5B6B7C] flex items-start space-x-2">
      ...
      {/* 5 Banking Pillar Breakdown & F-10 Explainability Drill-Down */}
        ...
        <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
          {pillarEvidences.map((pillar, idx) => (
            ...
            >
              ...
              {/* Mini Chart if pillar has chart data */}
              {pillar.chartData && (
      ...
      {/* Verified 6-Month UBL Cashflow Chart (GR-2 & GR-3) */}
      ...
      {/* F-11 RM Decision Capture & Official Audit Notes */}
      <div className="bg-white rounded-2xl p-6 border border-[#E4EBF2] shadow-sm space-y-4">
        ...
        <div className="space-y-4">
          ...
          <div className="flex flex-wrap items-center justify-end gap-3 pt-2">
            <button
              ...
              className="px-4 py-2.5 bg-slate-900 hover:bg-slate-950 text-white font-bold text-xs rounded-xl transition-all flex items-center justify-center space-x-1.5 border border-slate-800"
            >
            ...
            <button
              ...
              className="px-4 py-2.5 bg-[#D6455B] hover:bg-[#D6455B]/90 text-white font-bold text-xs rounded-xl transition-all flex items-center justify-center space-x-1.5"
            >
            ...
            <button
              ...
              className="px-4 py-2.5 bg-[#E8A33D] hover:bg-[#E8A33D]/90 text-white font-bold text-xs rounded-xl transition-all flex items-center justify-center space-x-1.5"
            >
              <FileText className="w-4 h-4" />
              <span>Counter-Offer (PKR {eligibility.recommendedAmount.toLocaleString()})</span>
            </button>

            <button
              onClick={() => handleRmDecision("APPROVED")}
              className="px-5 py-2.5 bg-[#1E9E5A] hover:bg-[#1E9E5A]/90 text-white font-extrabold text-xs rounded-xl transition-all flex items-center justify-center space-x-1.5"
            >
