...
function getSmeData(sme: SMEProfile) {
  ...
  const onTimePaymentRate = sme.onTimePaymentRate ?? sme.paymentRegularity ?? 100;
  const digitalTxnShare = sme.digitalTxnShare ?? 50;
  const uniqueCounterparties = sme.uniqueCounterparties ?? 10;
  return { monthlyInflows, monthlyOutflows, currentBalance, onTimePaymentRate, digitalTxnShare, uniqueCounterparties };
}

// ─── PRD F-6: 5 Banking Question Pillars ───

function scoreCashflowStability(sme: SMEProfile): PillarEvidence {
  const { monthlyInflows, monthlyOutflows } = getSmeData(sme);
  const nets = monthlyInflows.map((v, i) => v - (monthlyOutflows[i] || 0));
...
function scoreLiquidity(sme: SMEProfile): PillarEvidence {
  const { monthlyOutflows, currentBalance } = getSmeData(sme);
  const avgOutflow = mean(monthlyOutflows);
  const reserveMonths = currentBalance / (avgOutflow || 1);

  const raw = 20 * (Math.min(reserveMonths, 1.5) / 1.5);
  const score = clamp(raw, 0, 20);

  const evidence: string[] = [
    `Current UBL liquid balance: PKR ${currentBalance.toLocaleString()}`,
...
function scoreBusinessBehaviour(sme: SMEProfile): PillarEvidence {
  const { onTimePaymentRate, digitalTxnShare, uniqueCounterparties } = getSmeData(sme);
  const paymentScore = (onTimePaymentRate / 100) * 10;
  const digitalScore = (digitalTxnShare / 100) * 5;
  const score = clamp(paymentScore + digitalScore, 0, 15);

  const evidence: string[] = [
    `On-time supplier payment track record: ${onTimePaymentRate}%`,
    `Digital transaction share: ${digitalTxnShare}% of total volume`,
    `Unique counterparties: ${uniqueCounterparties} active partners`,
  ];
  if (onTimePaymentRate >= 90) evidence.push("✓ Excellent payment discipline");
  else if (onTimePaymentRate >= 70) evidence.push("⚠ Acceptable payment track record");
...
function scoreBusinessMomentum(sme: SMEProfile): PillarEvidence {
  const { monthlyInflows } = getSmeData(sme);
  const inflows = monthlyInflows;
