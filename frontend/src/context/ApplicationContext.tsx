...
export function ApplicationProvider({ children }: { children: React.ReactNode }) {
  ...
  const [askedLoan, setAskedLoan] = useState<number>(SAMPLE_SME_PROFILES[0].defaultAskedLoan || 1000000);
  const [tenureMonths, setTenureMonths] = useState<number>(SAMPLE_SME_PROFILES[0].defaultAskedTenure || 6);
  const [purpose, setPurpose] = useState<string>("Inventory Stocking");

  const [scoreResult, setScoreResult] = useState<ScoreEngineResult>(() =>
    calculateCashPulseScore(SAMPLE_SME_PROFILES[0], SAMPLE_SME_PROFILES[0].defaultAskedLoan || 1000000, SAMPLE_SME_PROFILES[0].defaultAskedTenure || 6)
  );
  ...
  useEffect(() => {
    const defaultLoan = selectedSme.defaultAskedLoan || 1000000;
    const defaultTenure = selectedSme.defaultAskedTenure || 6;
    setAskedLoan(defaultLoan);
    setTenureMonths(defaultTenure);
